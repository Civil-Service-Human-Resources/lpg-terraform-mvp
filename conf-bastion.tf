...
resource "azurerm_network_interface" "bastion_nic" {
  name                = "${var.rg_prefix}-${var.bastion_name}-nic-${format("%02d", count.index+1)}"
  location            = "${lookup(var.zone, terraform.workspace)}"
  resource_group_name = "${azurerm_resource_group.rg.name}"

  ip_configuration {
    name                          = "${var.rg_prefix}-${var.bastion_name}-ipconfig"
...
resource "azurerm_virtual_machine" "bastion_vm" {
  name                  = "${var.rg_prefix}-${var.bastion_name}vm-${format("%02d", count.index+1)}"
  location              = "${lookup(var.zone, terraform.workspace)}"
  resource_group_name   = "${azurerm_resource_group.rg.name}"
  ...
  storage_os_disk {
    name              = "${var.bastion_name}-osdisk-${format("%02d", count.index+1)}"
  ...
  storage_data_disk {
    name              = "${azurerm_managed_disk.bastion_datadisk.name}"
    managed_disk_id   = "${element(azurerm_managed_disk.bastion_datadisk.*.id, count.index)}"
    managed_disk_type = "Standard_LRS"
    disk_size_gb      = "10"
  ...
  os_profile {
    computer_name  = "${var.bastion_name}-${format("%02d", count.index+1)}"
